...
- [x] 2.3 Run `shfmt -i 2 -w scripts/smoke-app.sh` then `shellcheck -x scripts/smoke-app.sh`; resolve all findings. Confirm `scripts/checks.sh` stays green — it discovers scripts via `git ls-files '*.sh'`, so the new files are picked up automatically.
