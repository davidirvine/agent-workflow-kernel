...
- [x] 5.1 Confirm the §2/§3/§4 changes travel correctly through `scripts/new-app.sh` and `scripts/sync-kernel.sh` (manifest/identity handling, app-owned-file rules), adjusting them if any changed file's generation/sync behavior is wrong. _(All six changed files are under `kernel.paths`, none are `appOwnedFiles` or `excludeFromGenerate`, and none carry donor identity — so they generate and sync unchanged. `generate-assert.sh` (structure + identity + sync round-trip) passes; no generator/sync adjustment needed.)_
- [x] 5.2 Add coverage proving a freshly generated app is not auto-registered and its push gate is not broken by a roborev hook re-arm (an assertion in `generate-assert.sh`/`smoke-app.sh` where feasible, otherwise a documented manual verification step). _(Added a hermetic, git-only posture section to `generate-assert.sh`: emitted post-commit is the no-op stub (not armed → not auto-registered), emitted checks.sh lints `.githooks` by the kernel-authored allowlist (no `.githooks/*` glob), emitted install-hooks.sh carries the neutralization guard, and a simulated re-arm confirms the roborev-managed hooks stay out of the lint input. Daemon-level non-registration is hook-driven, proven in the D1 audit.)_
...
- [x] 6.2 Run `/opsx:verify` for this change and confirm the implementation matches proposal/design/specs/tasks.
