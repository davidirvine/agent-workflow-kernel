...
### D7 — CI adds two new jobs alongside slice 1's lint job; smoke-app stays a stub; no kernel-level system tools added
...
**Kernel dependency posture (load-bearing):** the kernel root does NOT become a superset of every preset's dependencies. Each preset is self-contained — its own `package.json`, `package-lock.json`, and `node_modules/` (D1) — and CI runs `npm ci` _inside the preset directory_ so preset deps land in the preset's `node_modules`, never at the kernel root. The kernel root's `package.json` (slice 1) stays minimal: just prettier + plugins for the kernel's own files. System-level runtimes (Node from `.nvmrc`, already provisioned by slice 1's lint-format job) are the only things the kernel CI provides directly. If a future preset needs a system-level prereq the kernel does not already provide (e.g. a Rust toolchain), it goes in that preset's own job as a conditional setup step — never hoisted to the kernel root. For this slice's preset, no new system-level tool is added; FAUST is handled per D10.
...
### D10 — FAUST is the `@grame/faustwasm` npm package, invoked by a `prebuild` npm script before Vite

The preset declares `@grame/faustwasm` (the JavaScript-packaged FAUST compiler) as a regular npm dependency in its `package.json`, pinned via its own `package-lock.json`. The preset's `"prebuild"` npm script invokes the package's shipped compile script — concretely `node node_modules/@grame/faustwasm/scripts/faust2wasm.js faust/synth.dsp public/` — to produce the `public/` outputs Vite picks up (the WASM + JSON metadata). `npm run build` therefore runs `prebuild` → `vite build` atomically.

**The FAUST compiler is NOT a system tool the kernel installs.** Synth-d's `main` toolchain declares `@grame/faustwasm: ^0.16.1` (resolving to `0.16.1` in its lockfile, as of the synth-d HEAD captured in task 0.1) and never installs a native `faust` binary anywhere — CI included. The preset adopts the same posture: FAUST arrives transitively via `npm ci`, there is no `faust --version` to capture, no `$PATH` requirement, no native install step in CI, and no per-developer system prereq beyond Node+npm (the slice-1 `.nvmrc` already provisions Node). Slice 4's `setup.sh --check` therefore verifies the preset's lockfile resolves rather than verifying a system FAUST install.

**Why a npm `prebuild` script over a Vite plugin:** the `prebuild` lifecycle is built-in (no custom plugin to maintain), the compilation is one fan-out call to the FAUST package's shipped script, and any subsequent change can layer a Vite plugin later if HMR-on-DSP-edit becomes desirable. The reference DSP is small, so `prebuild` is fast; a `prebuild`-skip / cache flag is a later optimization if it becomes noticeable. **Alternative considered:** a native FAUST CLI provisioned in CI (an earlier framing of this decision) — rejected, because the donor toolchain has never used a native CLI, the npm package suffices, and going native would add a system-level prereq the kernel does not otherwise need — the exact "kernel becomes superset of preset deps" trap D7 names.
...
- **FAUST version drift between local devs and CI** → pinned via the preset's `package-lock.json` (`@grame/faustwasm` is a regular npm dep per D10); `npm ci` enforces the lockfile in CI and locally; slice 4's `setup.sh --check` will assert the lockfile resolves rather than verifying a system FAUST install. No native FAUST install exists to drift.
...
- **FAUST pinned version:** ~~match what synth-d currently uses (need a quick `faust --version` capture from the synth-d toolchain); confirm before task 11.~~ **Resolved (D10):** synth-d's `main` declares `@grame/faustwasm: ^0.16.1` (resolving to `0.16.1` in its lockfile). The preset declares the same dep; the resolved version is captured at install time in the preset's own `package-lock.json`. Task 0.1 records the synth-d HEAD SHA used as the source of truth; no separate "FAUST pin" lives in `STACK.md` (the npm lockfile is the pin).
