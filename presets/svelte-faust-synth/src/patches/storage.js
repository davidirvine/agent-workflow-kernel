...
//                                   for what exists; a stray slot key cannot
//                                   create a phantom entry).
//   - `<namespace>patch:<name>`   → JSON envelope `{ name, version, params }`.
//
 ...
 */
export function createPatchStorage(namespace) {
  const INDEX_KEY = namespace + 'patches'
  const SLOT_PREFIX = namespace + 'patch:'
   ...
   * name is already upper-case it does nothing. Runs as part of listPatches so
   * the UI (which lists before any load/delete/rename) always operates on
  ...
  /**
   * Save the current params as a named patch. Only in-scope params
   * (PARAM_NAMES) are serialized; anything else in `params` (e.g. modWheel) is
   * excluded. A name collision overwrites the existing slot. Storage failures
   * are non-fatal.
   * @param {string} name
  ...
  function savePatch(name, params) {
    ...
    if (!index.includes(clean)) {
      index.push(clean)
      if (!writeIndex(index)) {
        // The slot was written but the index update failed (e.g. quota). Roll
        // back the orphaned slot so it can't linger unreferenced by listPatches.
  ...
  function renamePatch(oldName, newName) {
    ...
    // Replace `from` with `to` at its position; drop any pre-existing `to`
    // entry (overwrite) so the index has no duplicate. Preserve order otherwise.
    const index = readIndex()
    /** @type {string[]} */
    const next = []
    ...
    // Write the index FIRST: if it fails, nothing has changed and no slot data
    // is touched. (Ordering mirrors deletePatch and avoids destroying an
    // overwritten target's data on a later rollback.)
    if (!writeIndex(next)) return { ok: false, error: 'storage-unavailable' }

    // Then write the renamed slot. setItem is all-or-nothing per key, so a
    // failure here leaves the (possibly pre-existing) target slot untouched —
    // restore the original index and bail without having destroyed any patch's
    // data.
  ...
  return {
    ...
    deletePatch,
    renamePatch,
    // Keys are exposed for testability — tests assert against the exact keys
    // an instance writes. NOT for runtime use; consumers should call the
    // API methods, not read these directly.
    _indexKey: INDEX_KEY,
    _slotPrefix: SLOT_PREFIX,
  }
}
