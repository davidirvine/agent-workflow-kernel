...
- `new-app.sh` — generator: emit a fresh app from the kernel + a chosen `--preset`
- `sync-kernel.sh` — re-pull kernel files into an existing app (clobber-protected, kernel-version-stamped)
...
- smoke-app CI, the kernel's `.githooks`, and `release-please` config
