...
**Gate:** (a) manifest-validate — the generated sync manifest is well-formed and every tier-tagged spec is accounted for; (b) identity-leak / chassis-purity check — no `synth-d` package name, `synth-d:` namespace, GitHub URL, or deploy path survives unparameterized (the executable form of the slice's "watch for", catching the _unknown_ leaks the Phase-1 `MidiCcMap` finding implies exist); (c) `npm ci && npm build` succeeds on the bare preset.
